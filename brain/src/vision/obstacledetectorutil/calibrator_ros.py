...
class CalibratorRos:
    ...
    def __init__(self, source='kinect', visuals=False, filesource='default', caldestination='default'):
        ...
        self.rgb_retreiver = util.rosimage.RosImage("/camera/rgb/image_color", "passthrough")
        self.depth_retreiver = util.rosimage.RosImage("/camera/depth_registered/image_raw", "passthrough")
    ...
    def visualCalibrate(self, frames = 30, test=0):
        ...
        while True:
            ...
            if self.source == 'kinect':
                ...
                if image == None:
                    print "No image received in %f seconds, crashing!" % (try_sleep * try_count)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == 'quick':
            # create calibrator
            cator = calibrator.CalibratorRos()
            cator.calibrate()
        else:
            print "USAGE:"
        ...
        cator = calibrator.CalibratorRos(visuals=True, source='kinect')
