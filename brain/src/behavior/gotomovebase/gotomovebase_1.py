...
class GotoMoveBase_x(basebehavior.behaviorimplementation.BehaviorImplementation):
    ...
    def implementation_update(self):
        ...
        if self.m.n_occurs('goto') > 0:
            ...
            if recogtime > (time.time() - self.timeGuard):  # Was the command given in the last X seconds?
                if 'new_goal' in properties and self.goalLocation != properties['new_goal'] and self.goalLocationName != \
                        properties['new_goal']:
                    ...
                    if 'align_to_point' in properties and 'x' in properties['align_to_point'] and 'y' in properties[
                        'align_to_point']:
            ...
            if recogtime > (time.time() - self.timeGuard):  # was the command given in the last X seconds?
                if 'status' in properties and self.lastStatus == "goal_received":
                    ...
                    if self.lastStatus == "goal_reached":
                        ...
                        self.m.add_item('goto', time.time(),
                                        {'status': 'goal_cancelled', 'goal_name': self.goalLocationName,
                                         'goal': self.goalLocation})
                        # Reset goal
                        self.reset_goal()
                        return
                ...
                print "Goal reached"
                self.m.add_item('goto', time.time(), {'status': 'goal_reached', 'goal_name': self.goalLocationName,
                                                      'goal': self.goalLocation})
                # Reset goal
                self.reset_goal()
                return
    ...
    def check_alignment(self):
        ...
        if current_location == None:
            self.logger.warning(
                "Can't get current orientation as there are no odometry readings yet. Can't rotate to goal")
            # self.isAligned = True
            return
